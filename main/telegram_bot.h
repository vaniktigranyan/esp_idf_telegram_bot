typedef struct {
    ...
    char chat_id[32];           // Default chat ID to send messages
    char last_message[512];     // Last received message text
    int64_t last_update_id;     // ID of the last update (used in getUpdates)
    ...
    char proxy_url[128];        // Proxy server URL (if used)
    int proxy_port;             // Proxy port number
    bool use_ssl;               // Use HTTPS (true) or HTTP (false)
} telegram_bot_info_t;
